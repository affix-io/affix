...
import (
	...
	p2ptest "github.com/affix-io/affix/p2p/test"
...
func TestDatasetRequestsListP2p(t *testing.T) {
	...
	factory := p2ptest.NewTestNodeFactory(p2p.NewTestableaffixNode)
	...
	peers := make([]*p2p.affixNode, len(testPeers))
	for i, node := range testPeers {
		peers[i] = node.(*p2p.affixNode)
	}

	var wg sync.WaitGroup
	for _, p1 := range peers {
		wg.Add(1)
		go func(node *p2p.affixNode) {
			defer wg.Done()
