...
import (
	...
	apispec "github.com/affix-io/affix/api/spec"
	...
	"github.com/affix-io/affix/logbook"
...
func newTestRepo(t *testing.T) (r repo.Repo, teardown func()) {
	...
	teardown = func() {
		golog.SetLogLevel("affixapi", "info")
...
func newTestNode(t *testing.T) (node *p2p.affixNode, teardown func()) {
	t.Helper()

	var r repo.Repo
	r, teardown = newTestRepo(t)
	node, err := p2p.NewaffixNode(r, testcfg.DefaultP2PForTesting(), event.NilBus, nil)
	if err != nil {
		t.Fatal(err.Error())
	}
...
func newTestInstanceWithProfileFromNode(ctx context.Context, node *p2p.affixNode) *lib.Instance {
	return newTestInstanceWithProfileFromNodeAndOrchestratorOpts(ctx, node, nil)
}

func newTestInstanceWithProfileFromNodeAndOrchestratorOpts(ctx context.Context, node *p2p.affixNode, o *automation.OrchestratorOptions) *lib.Instance {
	cfg := testcfg.DefaultConfigForTesting()
...
func confirmaffixNotRunning() error {
	...
	if err != nil {
		return fmt.Errorf("it looks like a affix server is already running on address %s, please close before running tests", config.DefaultAPIAddress)
	}
